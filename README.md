...
4. Injects this EM into `CCTC_REDCap_Docker/redcap_source/modules/enhance_reason_for_change_v1.0.1` and brings the Docker stack up (`app`, `db`, `mailhog`).
...
6. Builds the spec list from `automated_tests/E.125.*.feature` (excluding `*REDUNDANT*`) and runs them via `npm run test:retry-failed` (up to 3 attempts per spec, Chrome).
...
- `EM_NAME` / `EM_VERSION` — `enhance_reason_for_change` / `v1.0.1`. Bump `EM_VERSION` when releasing a new module version so the spec glob and inject path stay aligned.
