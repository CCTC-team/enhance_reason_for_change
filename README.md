...
4. Starts the AIO container (ports `8443`/`8025`, volume `cctc_mariadb_data`), bind-mounting **this commit's** EM over the image's `modules/enhance_reason_for_change_v1.0.1` so REDCap serves the code under test with no rebuild.
...
- `publish-report` — merges the run's mochawesome JSON into one combined HTML report and publishes it to GitHub Pages (report named `enhance_reason_for_change_v1.0.1.html`, also served at the Pages root as `index.html`).
...
- `EM_NAME` / `EM_VERSION` — `enhance_reason_for_change` / `v1.0.1`. `EM_MODULE` (`enhance_reason_for_change_v1.0.1`) is the directory REDCap discovers the module by and the runner uses to locate the specs. Bump `EM_VERSION`/`EM_MODULE` when releasing a new module version so the mount path and spec discovery stay aligned.
